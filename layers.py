...
def softmaxloss_backward(x, labels):
    ...
    Returns:
    - dldx (numpy.ndarray): Partial derivative of the loss with respect to x. Average over the batch elements as in the forward pass.

    Raises:
    - ValueError: If the size of x and labels are not compatible for matrix multiplication.
    
    Note:
    - Labels here are in [0, 1, 2, ..., 9]. For both mnist and Cifar10 dataset
    """
    ...
    y[labels,np.arange(batch)]-=1 #np.arange(batch) makes column of length #samples in batch
    dldx = y/batch
...
def softmaxloss_forward(x, labels):
    ...
    Returns:
    - L (float): Computed loss. You should average over the batch elements.
    ...
    x_c = x[labels, np.arange(batch)] # Gives me the value on the correct postiton. label is the row and colon is the sample
    # Implement here
